...
class Dataset_ETT_hour(Dataset):
    ...
    def __len__(self): # 전체 데이터에서 앞 부분은 seq_len만큼 입력으로 들어가고 마지막 pred_len은 예측해야 하므로 데이터 셋에 포함하면 안됨. 따라서 이 둘을 제외한 데이터 셋이 유효한 시퀀스의 길이가 됨.
        return len(self.data_x) - self.seq_len- self.pred_len + 1 # 전체 데이터에서 sequence 길이와 predict 길이를 뺀 길이 = 유효한 sequence를 예측하는 횟수
